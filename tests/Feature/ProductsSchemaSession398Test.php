...
it('registers the plugin migration path on the migrator', function () {
    // Extracted (session 398): the provider's loadMigrationsFrom(__DIR__ …)
    // resolves to the real vendor path — re-pointed from the in-repo path at
    // extraction (the 395 symlink nuance).
    expect(app('migrator')->paths())->toContain(base_path('vendor/nonprofitcrm/products/database/migrations'));
});
